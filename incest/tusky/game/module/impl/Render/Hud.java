    ...
    public void onRender(EventRender2D eventRender2D) {
        if (waterMark.getCurrentValue()) {
            switch (waterMarkMode.currentMode) {
                case "Minced": {
                    DragWaterMark pon = (DragWaterMark) tuskevich.instance.draggableHUD.getDraggableComponentByClass(DragWaterMark.class);
        ...
        if (potions.getCurrentValue()) {
            ...
            DragSessionInfo dsi = (DragSessionInfo) tuskevich.instance.draggableHUD.getDraggableComponentByClass(DragSessionInfo.class);
            dsi.setWidth(155);
            dsi.setHeight(56);
            GLUtils.INSTANCE.rescale(scale);
            ...
            String name = tuskevich.instance.featureManager.getFeature(NameProtect.class).isEnabled() && NameProtect.myName.getCurrentValue() ? "" + "Protected" : " " + Helper.mc.player.getName();
            String time;
