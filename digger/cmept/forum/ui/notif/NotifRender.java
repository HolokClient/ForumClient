package digger.cmept.forum.ui.notif;

import digger.cmept.forum.module.impl.Render.Notifications;
...
public final class NotifRender implements Helper {
    ...
    public static void publish(ScaledResolution sr) {
        if (forum.instance.featureManager.getFeature(Notifications.class).isEnabled() && Notifications.notifMode.currentMode.equalsIgnoreCase("Rect") && !mc.gameSettings.showDebugInfo && mc.world != null && !(mc.currentScreen instanceof guiscreencomponent)) {
            if (!NOTIFICATIONS.isEmpty()) {
                ...
                for (Notif notification : NOTIFICATIONS) {
                    ...
                    if (!notification.getTimer().hasReached(notification.getTime() / 2)) {
                        ...
                        notification.notificationTimeBarWidth = MathHelper.EaseOutBack((float) notification.notificationTimeBarWidth, 0, (float) (4 * forum.deltaTime()));
                    }

                    if (!notification.getTimer().hasReached(notification.getTime())) {
                        notification.x = sr.getScaledWidth() - 185;
                        notification.y = MathHelper.EaseOutBack((float) notification.y, (float) y, (float) (5 * forum.deltaTime()));
                    } else {
                        notification.x = sr.getScaledWidth() - 185;
